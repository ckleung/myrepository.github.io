...
//	char* m_pchData;
  ...
  double  Double() const        { return atof(GetString()); }
  float Float() const       { return float(atof(GetString())); }
  int   Int() const         { return atoi(GetString()); }
  char  Char() const        { return GetAt(0); }
  bool  Bool() const        { return (Int() != 0); }
  _variant_t Variant() const{return atov(GetString());}
  COleCurrency Currency() const{return atoCur(GetString());}
};
